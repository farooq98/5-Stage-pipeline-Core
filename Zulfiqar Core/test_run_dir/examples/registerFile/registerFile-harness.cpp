...
#endif
#include <iostream>
class registerFile_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    registerFile_api_t(VregisterFile* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_rd_sel)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_rs2_sel)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_rs1_sel)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_regWrite)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_rs2)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_rs1)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["registerFile.reset"] = 0;
    }
#if VM_TRACE
...
static registerFile_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    std::string vcdfile = "test_run_dir/examples/registerFile/registerFile.vcd";
