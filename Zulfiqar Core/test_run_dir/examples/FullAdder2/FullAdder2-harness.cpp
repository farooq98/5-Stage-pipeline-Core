...
#endif
#include <iostream>
class FullAdder2_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    FullAdder2_api_t(VFullAdder2* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_b2)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_a2)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_b1)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_a1)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_cin)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_cout)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_sum1)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_sum0)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["FullAdder2.reset"] = 0;
    }
#if VM_TRACE
...
static FullAdder2_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    VFullAdder2* top = new VFullAdder2;
    std::string vcdfile = "test_run_dir/examples/FullAdder2/FullAdder2.vcd";
