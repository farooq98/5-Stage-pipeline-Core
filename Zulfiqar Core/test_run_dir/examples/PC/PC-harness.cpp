...
#endif
#include <iostream>
class PC_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    PC_api_t(VPC* _dut) {
        dut = _dut;
        main_time = 0L;
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_input)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_pc)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_pc4)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["PC.reset"] = 0;
    }
#if VM_TRACE
...
int main(int argc, char **argv, char **env) {
    ...
    VPC* top = new VPC;
    std::string vcdfile = "test_run_dir/examples/PC/PC.vcd";
