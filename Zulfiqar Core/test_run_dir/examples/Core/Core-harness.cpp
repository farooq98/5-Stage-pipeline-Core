...
#endif
#include <iostream>
class Core_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    Core_api_t(VCore* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_branchCheck)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_AluOut)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_instruction)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["Core.reset"] = 0;
    }
#if VM_TRACE
...
int main(int argc, char **argv, char **env) {
    ...
    VCore* top = new VCore;
    std::string vcdfile = "test_run_dir/examples/Core/Core.vcd";
